...
# Pretraining models for SSL methods
...
class Custom2DCNN(nn.Module):
    '''
    Description: A multi-task 2D CNN model for extracting frame features from full/kidney/liver/spleen images.
    Usage: It is only used in 2D pretraining scheme, which only 2D CNN is pretrained. 
           The pretrained weights are later used in downstream task.
    '''
    def __init__(self, hidden = 368, num_channel = 3):
        ...
        self.full_extractor.fc = torch.nn.Identity()
        self.kidney_extractor.fc = torch.nn.Identity()
        self.liver_extractor.fc = torch.nn.Identity()
        self.spleen_extractor.fc = torch.nn.Identity()
    ...
    def forward(self, full_input, crop_kidney, crop_liver, crop_spleen):
        full_output = self.full_extractor(full_input) # output: (1, img_num // num_channel, 368)
        ...
        full_projection = self.full_projector(full_output.squeeze(0))
        kidney_projection = self.kidney_projector(kidney_output.squeeze(0))
        liver_projection = self.liver_projector(liver_output.squeeze(0))
        spleen_projection = self.spleen_projector(spleen_output.squeeze(0))
...
# Downstream model for supervised learning
class ContextProcessor(nn.Module):
    '''
    Aggregate frame embeddings to a video embedding.
    '''
...
class Custom3DCNN(nn.Module):
    '''
    Description: A multi-task 3D CNN model for extracting video features from full/kidney/liver/spleen videos.
    Usage: It is used in downstream, which only 3D CNN is pretrained. 
           The pretrained weights are later used in downstream task.
    '''
    def __init__(self, hidden = 368, num_channel = 3):
