...
class BarlowTwins(nn.Module):
    ...
    def forward(self, X): # X: (4, img_num, H, W)
        ...
        z1_all = self.online_encoder(*[self.augment_fn1(x) for x in X]) # x_all: (4, img_num, proj_dim)
        z2_all = self.online_encoder(*[self.augment_fn2(x) for x in X]) # y_all: (4, img_num, proj_dim)
        
        loss_all = []
        for z1, z2 in zip(z1_all, z2_all): # x, y: (img_num, proj_dim)
