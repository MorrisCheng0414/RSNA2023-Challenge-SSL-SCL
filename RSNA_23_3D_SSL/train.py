...
from ssl_methods.unimoco import UniMoCo
...
def specificity_score(y_true, y_pred):
    mcm = multilabel_confusion_matrix(y_true, np.where(y_pred > 0.5, 1, 0)) # CM: (TN, FP, 
                                                                            #     (FN, TP)
    spec = mcm[:, 0, 0] / (mcm[:, 0, 0] + mcm[:, 0, 1]) # TN / (TN + FP)
...
def pretrain_function(model,
    ...
    pretrain_loss = 0.
    for step_idx, sample in enumerate(pbar := tqdm(loader), start = 1):
        # Load batch data
        ...
        Y = torch.stack([bowel | extravasation, kidney, liver, spleen], dim = 1).to(device)

        # Forward pass
        with torch.amp.autocast(device_type = "cuda", dtype = torch.bfloat16):
            if isinstance(model, TiCo): model.momentum_update_key_encoder(tico_m) # model.__name__ == 'TiCo'

            if isinstance(model, UniMoCo): # UniMoCo with supervise signal
                losses = model(X, Y)
            else:                          # self-supervised learning
                losses = model(X)

        # Record loss
        for idx, loss in enumerate(losses): # losses: (4)
