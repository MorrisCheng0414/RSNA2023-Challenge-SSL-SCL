...
class VICReg(nn.Module):
    ...
    def forward(self, X): # X: (4, batch_size, img_num, H, W)
        ...
        x_all = self.online_encoder(*[self.augment_fn1(x) for x in X]) # x_all: (4, batch_size, proj_dim)
        y_all = self.online_encoder(*[self.augment_fn2(x) for x in X]) # y_all: (4, batch_size, proj_dim)
        
        loss_all = []
        for x, y in zip(x_all, y_all): # x, y: (batch_size, proj_dim)
