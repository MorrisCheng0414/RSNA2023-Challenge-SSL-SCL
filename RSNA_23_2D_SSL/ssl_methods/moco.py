...
class MoCo(nn.Module):
    def __init__(self):
        ...
        self.proj_dim = 256 # default: 256

        self.queue_size = 20480 # default: 640 * 32 = 20480
    ...
    def batch_shuffle(self, x):
        idx_shuffle = torch.randperm(x[0].shape[0]) # x: (4, img_num, H, W)
        idx_unshuffle = torch.argsort(idx_shuffle)
        return [x[i][idx_shuffle, :, :] for i in range(len(x))], idx_unshuffle
     
    @torch.no_grad()
    def batch_unshuffle(self, x, idx_unshuffle): # x: (4, img_num, dim)
