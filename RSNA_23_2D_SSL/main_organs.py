...
from ssl_methods import BarlowTwins, BYOL, MoCo, TiCo, VICReg, SimSiam
...
if __name__ == "__main__":
    ...
    model_map = {"barlowtwins": BarlowTwins, "byol": BYOL, "moco": MoCo, 
                 "tico": TiCo, "vicreg": VICReg, "simsiam": SimSiam}
    modes = ["barlowtwins", "byol", "moco", "tico", "vicreg", "simsiam"]
    for mode in modes:
        if mode not in model_map: 
            raise ValueError(f"Invalid mode: {mode}")

    # # Clear weights
    # clear_folds_weights("/kaggle/working/RSNA_23_2D_SSL/pretrain_weights")
    ...
    for k in range(k_fold):
        ...
        for mode in modes:
            fold_path = f"/kaggle/working/RSNA_23_2D_SSL/pretrain_weights/{mode}/fold{k+1}/"
            ...
            torch.save({"full_cnn": model.online_encoder.full_extractor.cnn.state_dict(),
                        "kidney_cnn": model.online_encoder.kidney_extractor.cnn.state_dict(),
                        "liver_cnn": model.online_encoder.liver_extractor.cnn.state_dict(),
                        "spleen_cnn": model.online_encoder.spleen_extractor.cnn.state_dict()}, 
