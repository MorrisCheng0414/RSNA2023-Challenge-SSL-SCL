...
from img_aug import LinclsAug
from utils import train_test_split

IMG_SIZE = 256
aug_function = LinclsAug()

def preprocess(n_splits = 5):
    train_series_meta = pd.read_csv("/kaggle/working/RSNA_23_VICReg/data/train_series_meta.csv")
    train_series_meta = train_series_meta.sort_values(by='patient_id').reset_index(drop=True)
    train = pd.read_csv("/kaggle/working/RSNA_23_VICReg/data/train.csv")
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        patient_series_path = os.path.join("/tf/ShareFromHostOS/RSNA_23_256_256_png", f"{patient_id}", f"{series_id}")
...
if __name__ == "__main__":
    ...
    for k in range(k_fold): 
        # Pretrain dataset
        pretrain_df = pd.concat([injury_folds[k][0], normal_folds[k][0]])
        print(pretrain_df.index.values)