...
def train_test_split(df:pd.DataFrame, test_size = 0.2):
    mask = np.random.rand(len(df)) < (1 - test_size)
    train = df[mask].reset_index(drop = True)
    test = df[~mask].reset_index(drop = True)
    return [train, test]

def weight_decay(moco_weight, supcon_weight, curr_step, total_step, warmup_ratio = 0.5, mode = 'cos'):
    warmup_step = int(total_step * warmup_ratio)

    if curr_step > warmup_step:
        if mode == 'cos':
            moco_weight = np.cos(((curr_step - warmup_step) / (total_step - warmup_step)) * (np.pi / 2))
            supcon_weight = 1 - moco_weight
        elif mode == 'linear':
            moco_weight = 1 - (curr_step - warmup_step) / (total_step - warmup_step)
            supcon_weight = 1 - moco_weight
        elif mode == 'exp':
            moco_weight = np.exp(-0.04 * (curr_step - warmup_step))
            supcon_weight = 1 - moco_weight
        elif mode == 'swap':
            moco_weight = 0.0
            supcon_weight = 1.0
    return moco_weight, supcon_weight

def plot_history(history, classes, title, folder_path):
    history = [np.array(history).flatten() for history in history]
    fig, ax = plt.subplots(1, 1, figsize = (10, 5))
    for idx, class_name in enumerate(classes):
        ax.plot(history[idx], label = class_name)
    ax.set_title(title)
    ax.set_xlabel('Step')
    ax.set_ylabel('Loss')
    ax.legend(loc = 'best')
    save_name = title.lower().replace(' ', '_')
    fig.savefig(os.path.join(folder_path, f"{save_name}.png"))

def clear_folds_weights(folder_path):
    weight_paths = glob(os.path.join(folder_path, "*", "*"))
    fold_paths = glob(os.path.join(folder_path, "*"))
